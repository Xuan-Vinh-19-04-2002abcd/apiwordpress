...
function author_portfolio_body_classes( $classes ) {
  ...
  <div id="div-comment-<?php comment_ID(); ?>" class="comment-body review-list">
	...
	<div class="single-comment">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<em><?php esc_html_e( 'Your comment is awaiting moderation.', 'author-portfolio' ); ?></em>
			<br />
		<?php endif; ?>
			<div class="comment-meta">
				...
				<h4><?php printf( wp_kses_post( '%s', 'author-portfolio' ), sprintf( '%s', get_comment_author_link() ) ); ?></h4>
			</div>
				<?php comment_text(); ?>
				<?php
				comment_reply_link(
...
endif; // ends check for author_portfolio_comment_list();

/**
 * Author VCard
 */
function author_portfolio_author_vcard() {
	?>
...
function author_portfolio_before_default_page_markup() {
	if (is_home()) {
...
add_action( 'author_portfolio_before_default_page', 'author_portfolio_before_default_page_markup' );
function author_portfolio_after_default_page_markup() {
...
add_action( 'author_portfolio_after_default_page', 'author_portfolio_after_default_page_markup' );

/**
 * Get Current Year
 */

 function author_portfolio_currentYear(){
    return date('Y');
}
 ...
 * Get theme Data
 */
function author_portfolio_author_uri(){
...
function author_portfolio_sbi_before_feed(){
	?>
	<div class="author-portfolio-instagram-section">
	<?php
}

add_action('sbi_after_feed', 'author_portfolio_sbi_after_feed');
function author_portfolio_sbi_after_feed(){
	?>
	</div>
	<?php
}
